...
export type Feedback = { id: string; title: string; description: string; status: 'new' | 'planned' | 'done'; createdAt: string };
  ...
  return <main>
    ...
    <section className="layout">
      ...
      <div className="feed">
        ...
        {!loading && !error && items.length === 0 && <div className="empty">No feedback yet. Add the first request.</div>}
        {/* {items.map(item =>
          <article key={item.id}>
            <div>
            ...
            <button className="secondary" onClick={() => void cycle(item.id)}>Move forward →</button>
          </article>)} */}
        {items.map(item =>
          <FeedbackCard key={item.id} feedback={item} onAdvance={() => void cycle(item.id)} ></FeedbackCard>
        )}
      </div>
    </section>
  </main>;
